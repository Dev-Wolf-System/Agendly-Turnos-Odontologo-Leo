        ...
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-5">
          {clinicas.map((clinica) => {
            ...
            return (
              <div
                key={clinica.id}
                className={`group relative overflow-hidden rounded-xl border border-[var(--border-light)] bg-card shadow-[var(--shadow-card)] transition-all duration-200 hover:border-[var(--ht-primary)]/30 hover:shadow-[var(--shadow-md)] hover:-translate-y-0.5 focus-within:ring-2 focus-within:ring-[var(--ht-primary)]/40 ${
                  !clinica.is_active ? "opacity-75" : ""
                }`}
              >
                {/* Overlay link — hace toda la card clickeable sin romper botones internos */}
                <Link
                  href={`/admin/clinicas/${clinica.id}`}
                  className="absolute inset-0 z-[1] outline-none"
                  aria-label={`Ver detalle de ${clinica.nombre}`}
                >
                ...
                <div className="relative p-5">
                  ...
                  {/* Actions — relative z-[2] para quedar por encima del Link overlay */}
                  <div className="relative z-[2] flex items-center gap-2 pt-3 border-t border-[var(--border-light)]">
                    {aprobacion === "pendiente" ? (
                      <>
                        <button
                          onClick={(e) => {
                            e.stopPropagation();
                            handleAprobar(clinica);
                          }}
                      ...
                      <>
                        <span className="flex-1 inline-flex items-center justify-center gap-1.5 rounded-lg bg-gradient-to-r from-[var(--ht-primary)] to-[var(--ht-accent-dark)] px-3 py-2 text-xs font-semibold text-white shadow-[var(--shadow-primary)] pointer-events-none">
